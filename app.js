...
function loadSprint() {
  try {
    ...
    return {
      focus: typeof parsedSprint.focus === "string"
        ? parsedSprint.focus
        : "",

      goals: typeof parsedSprint.goals === "string"
        ? parsedSprint.goals
        : "",

      tasks: Array.isArray(parsedSprint.tasks)
        ? parsedSprint.tasks
        : [],
    };
  } catch {
...
function renderProgress() {
  ...
  if (total === 0) {
    ...
    progressSummary.textContent =
      `All ${total} task${total === 1 ? "" : "s"} complete — great work!`;
  } else {
    progressSummary.textContent =
...
function renderFocusGuidance() {
  ...
  focusGuidance.classList.toggle(
    "is-guidance",
    !hasFocus
  );
}
...
taskList.addEventListener("change", (event) => {
  ...
  const task = sprint.tasks.find(
    (item) =>
      item.id === event.target.dataset.taskId
  );
